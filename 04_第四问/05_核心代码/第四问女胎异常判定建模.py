...
合同路径 = 项目目录 / "第四问题意合同与候选设计.md"

输出根目录 = 工作区 / "99_临时中转/当前四问一键复现/第四问"
数据输出目录 = 输出根目录 / "01_数据"
...
def 配置运行路径(
    ...
    global 工作簿路径, 题目路径, 合同路径
    global 输出根目录, 数据输出目录, 模型输出目录, 验证输出目录, 复现输出目录, 图表提示词目录
    工作簿路径 = workbook.resolve()
    题目路径 = problem_pdf.resolve()
    合同路径 = contract.resolve()
    输出根目录 = output_root.resolve()
    数据输出目录 = 输出根目录 / "01_数据"
    模型输出目录 = 输出根目录 / "02_模型结果"
    验证输出目录 = 输出根目录 / "03_验证"
    复现输出目录 = 输出根目录 / "04_复现"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="第四问女胎AB异常判定完整建模")
    parser.add_argument("--附件", type=Path, default=工作簿路径)
    parser.add_argument("--题目", type=Path, default=题目路径)
    parser.add_argument("--合同", type=Path, default=合同路径)
    parser.add_argument("--输出目录", type=Path, default=输出根目录)
    args = parser.parse_args()
    配置运行路径(args.附件, args.题目, args.合同, args.输出目录)
    main()
