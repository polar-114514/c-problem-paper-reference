...
def 定位工作区() -> Path:
    for 候选 in 本文件.parents:
        if 候选.name == "C题论文工作区":
            return 候选
    raise RuntimeError("无法从候选A脚本位置定位C题论文工作区")


源工作簿 = 定位工作区() / "00_题目与原始资料" / "02_原始数据" / "附件.xlsx"
...
def 生成报告(
    ...
    interval_crosses_zero = bootstrap_summary.loc[
        (bootstrap_summary["自助95CI下限_百分点"] <= 0)
        & (bootstrap_summary["自助95CI上限_百分点"] >= 0),
        "效应",
    ].astype(str).tolist()
    if interval_crosses_zero:
        self_denial = (
            "整簇自助95%区间跨越0的效应包括："
            + "、".join(interval_crosses_zero)
            + "。这些效应只解释为证据不足，不使用自行设定的方向保持率门槛；候选A仍需与其他候选比较组外误差、层级结构和非线性形态。"
        )
    else:
        self_denial = "核心效应的整簇自助95%区间均未跨越0；仍需与其他候选比较组外误差、层级结构和非线性形态。"
    lines = [
